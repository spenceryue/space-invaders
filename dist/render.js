...
import { setAt } from "./set-at.js";
export function render(state, rootElement) {
    ...
    aliens.forEach(({ column, kind, row }) => setAt(frame, { BONUS: ALIEN_BONUS, MINION: ALIEN_MINION }[kind], row, column));
    bullets.forEach(({ column, kind, row, /* TODO: Use for bullet trail? */ speed }) => setAt(frame, { ALIEN: ALIEN_BULLET, HUMAN: HUMAN_BULLET }[kind], row, column));
